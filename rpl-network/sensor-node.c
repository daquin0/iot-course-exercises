...
PROCESS_THREAD(sensor_node,ev,data)
{
    ...
    while (1){
        ...
        uint8_t is_node_rechable = NETSTACK_ROUTING.node_is_reachable();
        uint8_t got_server_adress = NETSTACK_ROUTING.get_root_ipaddr(&server_address);

        if(is_node_rechable  && got_server_adress){
